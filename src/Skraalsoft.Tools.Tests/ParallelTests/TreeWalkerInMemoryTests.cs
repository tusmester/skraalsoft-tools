...
using Skraalsoft.Tools.Parallel;
using System.Threading.Tasks;
using System.Collections.Concurrent;
using System.Linq;
using System.Diagnostics;

namespace Skraalsoft.Tools.Tests
{
    ...
    [TestClass]
    public class TreeWalkerInMemoryTests
    {
        //====================================================================== Visitor tests

        [TestMethod]
        public async Task TreeWalker_InMemory_EmptyLeaf()
        {
            var root = GetTree_EmptyLeaf();
            await TestWalk(root, 1, 3);
        }
        [TestMethod]
        public async Task TreeWalker_InMemory_EmptyContainer()
        {
            var root = GetTree_EmptyContainer();
            await TestWalk(root, 1, 2);
        }
        [TestMethod]
        public async Task TreeWalker_InMemory_OneLevel_OnlyContainers()
        {
            var root = GetTree_OneLevel_OnlyContainers();
            await TestWalk(root, 3, 2);
        }
        [TestMethod]
        public async Task TreeWalker_InMemory_OneLevel_OnlyLeaves()
        {
            var root = GetTree_OneLevel_OnlyLeaves();
            await TestWalk(root, 3, 2);
        }
        [TestMethod]
        public async Task TreeWalker_InMemory_OneLevel_Mixed()
        {
            var root = GetTree_OneLevel_Mixed();
            await TestWalk(root, 5, 2);
        }
        [TestMethod]
        public async Task TreeWalker_InMemory_TwoLevels_Balanced()
        {
            var root = GetTree_TwoLevels_Balanced();
            await TestWalk(root, 7, 2);
        }        
        [TestMethod]
        public async Task TreeWalker_InMemory_FourLevels_Unbalanced()
        {
        ...
        [TestMethod]
        public async Task TreeWalker_InMemory_TwoLevels_Balanced_Parallel()
        {
            var root = GetTree_TwoLevels_Balanced();
            await TestWalk(root, 7, 3, 1000, 4000, 4100);
        }
        [TestMethod]
        public async Task TreeWalker_InMemory_FourLevels_Unbalanced_Parallel()
        {
            var root = GetTree_FourLevels_Unbalanced_2();
            await TestWalk(root, 21, 5, 1000, 7000, 7100);
        }
        [TestMethod]
        public async Task TreeWalker_InMemory_DeepTree_Parallel_Sequential()
        {
        ...
        [TestMethod]
        public async Task TreeWalker_InMemory_WideTree_Parallel()
        {
